...
public enum BookmarkManager
{
  ...
  private static final Logger LOGGER = new Logger(Logger.Scope.MAIN, BookmarkManager.class);
  ...
  {
    LOGGER.d("Loading bookmarks file from: " + path);
  ...
  public boolean importBookmarksFile(@NonNull ContentResolver resolver, @NonNull Uri uri, @NonNull File tempDir)
  {
    ...
    if (filename == null)
    {
      LOGGER.w("Missing path in bookmarks URI: " + uri);
      return false;
    }

    LOGGER.w("Downloading bookmarks file " + uri);
    ...
    {
      LOGGER.w("Failed to download bookmarks file from " + uri, e);
      return false;
    }
