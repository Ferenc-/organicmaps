...
public enum LocationHelper implements Initializable<Context>, AppBackgroundTracker.OnTransitionListener, BaseLocationProvider.Listener
{
  ...
  private final LocationListener mCoreLocationListener = new LocationListener()
  {
    ...
    public void onLocationUpdated(Location location)
    {
      ...
      if (mInFirstRun)
      {
        LOGGER.d("Location update is obtained and must be ignored, because the app is in a first run mode");
        return;
      }
    ...
    {
      LOGGER.d("onLocationError errorCode = " + errorCode +
  ...
  private static final Logger LOGGER = new Logger(Logger.Scope.LOCATION, LocationHelper.class);
  @NonNull
      ...
      new LocationState.ModeChangeListener()
  {
    ...
    public void onMyPositionModeChanged(int newMode)
    {
      ...
      LOGGER.d("onMyPositionModeChanged mode = " + LocationState.nameOf(newMode));

      if (mUiCallback == null)
        LOGGER.d("UI is not ready to listen my position changes, i.e. it's not attached yet.");
    }
  };
  ...
  {
    LOGGER.d("Set stop location update by user: " + isStopped);
  ...
  public void onTransit(boolean foreground)
  {
    if (foreground)
    {
      LOGGER.d("Resumed in foreground");
    ...
    {
      LOGGER.d("Stopped in background");
  ...
  public void onLocationChanged(@NonNull Location location)
  {
    ...
    {
      LOGGER.w("Unsatisfied accuracy for location = " + location);
      return;
    }
    ...
    if (mSavedLocation != null)
    {
      ...
      {
        LOGGER.d("The new " + location + " is worse than the last " + mSavedLocation);
        return;
      }
    }
  ...
  {
    LOGGER.d("onLocationError(): " + errCode);
        ...
        !(mLocationProvider instanceof AndroidNativeProvider))
    {
      ...
      LOGGER.d("Downgrading to use native provider");
  ...
  {
    LOGGER.d("notifyMyPositionModeChanged(): " + LocationState.nameOf(newMode));
  ...
  {
    LOGGER.d("notifyLocationNotFound()");
    if (mUiCallback != null)
  ...
  {
    LOGGER.d("addListener(): " + listener);
    LOGGER.d(" - listener count was: " + mListeners.getSize());
  ...
  {
    LOGGER.d("removeListener(), listener: " + listener);
    LOGGER.d(" - listener count was: " + mListeners.getSize());
  ...
  {
    LOGGER.d("calcLocationUpdatesInterval()");
    if (RoutingController.get().isNavigating())
    {
      LOGGER.d("calcLocationUpdatesInterval(), it's navigation mode");
  ...
  public void start()
  {
    if (mActive)
    {
      LOGGER.w("Provider '" + mLocationProvider + "' is already started");
      return;
    }

    if (isLocationUpdateStoppedByUser())
    {
      LOGGER.d("Location updates are stopped by the user manually, so skip provider start"
    ...
    LOGGER.d("Old time interval (ms): " + oldInterval);
    calcLocationUpdatesInterval();
    if (!PermissionsUtils.isLocationGranted(mContext))
    {
      LOGGER.w("Dynamic permissions ACCESS_COARSE_LOCATION and/or ACCESS_FINE_LOCATION are granted");
      onLocationError(ERROR_DENIED);
      return;
    }
    LOGGER.i("start(): interval = " + mInterval + " provider = '" + mLocationProvider + "' mInFirstRun = " + mInFirstRun);
    checkForAgpsUpdates();
  ...
  public void stop()
  {
    ...
    if (!mActive)
    {
      LOGGER.w("Provider '" + mLocationProvider + "' is already stopped");
      return;
    }
  ...
  private void checkForAgpsUpdates()
  {
    ...
    {
      LOGGER.d("A-GPS should be up to date");
      return;
    }

    LOGGER.d("Requesting new A-GPS data");
  ...
  public void attach(@NonNull UiCallback callback)
  {
    ...
    if (mUiCallback != null)
    {
      LOGGER.d(" - already attached. Skip.");
      return;
    }
    ...
    if (mActive)
    {
      LOGGER.d("attach() provider '" + mLocationProvider + "' is active, just add the listener");
      if (mSavedLocation != null)
  ...
  {
    LOGGER.d("detach(), delayed: " + delayed);

    if (mUiCallback == null)
    {
      LOGGER.d(" - already detached. Skip.");
      return;
    }
  ...
  {
    LOGGER.i("onEnteredIntoFirstRun");
    mInFirstRun = true;
  }
  ...
  public void onExitFromFirstRun()
  {
    ...
    {
      notifyLocationUpdated();
      LOGGER.d("Current location is available, so play the nice zoom animation");
