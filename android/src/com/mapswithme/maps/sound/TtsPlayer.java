...
public enum TtsPlayer implements Initializable<Context>
{
  ...
  private static final Logger LOGGER = new Logger(Logger.Scope.MAIN, TtsPlayer.class);
  ...
  public void initialize(@Nullable Context context)
  {
    ...
    mTts = new TextToSpeech(context, status -> {
      ...
      {
        LOGGER.w("Failed to initialize TextToSpeach");
        lockDown();
        mInitializing = false;
        return;
      }
  ...
  private boolean getUsableLanguages(List<LanguageData> outList)
  {
    ...
    for (int i = 0; i < codes.length; i++)
    {
      try
      {
        ...
        LOGGER.w("Failed to get usable languages " + ignored.getMessage());
      }
      catch (IllegalArgumentException e)
      {
        LOGGER.e("Failed to get usable languages", e);
        lockDown();
        return false;
      }
    }
