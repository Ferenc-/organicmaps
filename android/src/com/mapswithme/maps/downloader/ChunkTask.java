...
{
  private static final Logger LOGGER = new Logger(Logger.Scope.DOWNLOADER, ChunkTask.class);
  ...
  protected Integer doInBackground(Void... p)
  {
    ...
    try
    {
      ...
      if ((isChunk && err != HttpURLConnection.HTTP_PARTIAL) || (!isChunk && err != HttpURLConnection.HTTP_OK))
      {
        ...
        LOGGER.w("Error for " + urlConnection.getURL() +
    ...
    {
      LOGGER.e("Invalid url: " + mUrl, ex);
      return INVALID_URL;
    } catch (final IOException ex)
    {
      LOGGER.d("IOException in doInBackground for URL: " + mUrl, ex);
      return IO_EXCEPTION;
    } finally
    {
  ...
  private Integer downloadFromStream(InputStream stream)
  {
    ...
    for (int size : arrSize)
    {
      ...
      {
        LOGGER.e("IOException in downloadFromStream for chunk size: " + size, ex);
      }
    }
