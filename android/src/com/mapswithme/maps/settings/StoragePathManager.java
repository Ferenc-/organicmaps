...
{
  private static final Logger LOGGER = new Logger(Logger.Scope.STORAGE, StoragePathManager.class);
  ...
  private void addStorageOption(File dir, boolean isInternal, String configPath)
  {
    ...
    if (dir == null)
    {
      LOGGER.w("The system returned 'null' " + (isInternal ? "internal" : "external") + " storage");
      return;
    }
    ...
    {
      LOGGER.e("IOException at getCanonicalPath for " + dir.getPath(), e);
      return;
    }
    ...
    if (!isInternal)
    {
      ...
      catch (IllegalArgumentException e)
      {
        ...
        LOGGER.w("External storage checks failed for " + commentedPath);
      }
      ...
      if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N)
      {
        ...
        if (sm != null)
        {
          ...
          if (sv != null)
          {
            ...
            LOGGER.w("Can't get StorageVolume for " + commentedPath);
        }
        else
          LOGGER.w("Can't get StorageManager for " + commentedPath);
      }
    }
    ...
    {
      LOGGER.w("Not mounted: " + commentedPath);
      return;
    }

    if (!dir.exists())
    {
      LOGGER.w("Not exists: " + commentedPath);
      return;
    }
    if (!dir.isDirectory())
    {
      LOGGER.w("Not a directory: " + commentedPath);
      return;
    }
    ...
    LOGGER.i("Accepted " + commentedPath);
  }
  ...
  public void scanAvailableStorages() throws AssertionError
  {
    ...
    LOGGER.i("Currently configured storage: " + (TextUtils.isEmpty(configPath) ? "N/A" : configPath));

    LOGGER.i("Begin scanning storages");
    mStorages.clear();
    ...
    LOGGER.i("End scanning storages");
    ...
    {
      LOGGER.w("Currently configured storage is not available!");
    }
  }
  ...
  public static String findMapsStorage(@NonNull Application application)
  {
    ...
    if (currentIdx != -1)
    {
      ...
      {
        LOGGER.i("Found map files at the currently configured " + path);
        return path;
      }
      else
      {
        LOGGER.w("No map files found at the currenly configured " + path);
      }
    }

    LOGGER.i("Looking for map files in available storages...");
    for (int idx = 0; idx < storages.size(); ++idx)
    {
      ...
      if (containsMapData(path))
      {
        LOGGER.i("Found map files at " + path);
        return path;
      }
      else
      {
        LOGGER.i("No map files found at " + path);
      }
    }

    path = mgr.getDefaultStorage().mPath;
    LOGGER.i("Using default storage " + path);
    return path;
  }
  ...
  {
    LOGGER.i("Begin moving maps from " + oldPath + " to " + newPath);
    ...
    {
      LOGGER.i("Moving " + oldFiles[i].getPath() + " to " + newFiles[i].getPath());
      File parent = newFiles[i].getParentFile();
      if (parent != null)
        parent.mkdirs();
      if (!MapManager.nativeMoveFile(oldFiles[i].getPath(), newFiles[i].getPath()))
      {
        LOGGER.e("Failed to move " + oldFiles[i].getPath() + " to " + newFiles[i].getPath());
